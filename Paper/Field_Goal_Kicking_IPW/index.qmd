...
<!-- DRAFT — PENDING FINAL PASS. Do not treat as camera-ready.
  ...
  ALSO PENDING (numbers, not prose):
   - Log-loss cells in @tbl-model-metrics and @tbl-m3-grid are marked TBD and must be
     filled from a verified local run of 04_evaluation.ipynb.
   - The humidity term now shown in the §2.2 covariate list and the §4.1 M1 formula is
     the specified model; all numeric results currently in the paper predate that refit
...
$$\begin{aligned}
\text{logit}\bigl(P(Y_i=1 \mid X_i)\bigr) &= \sum_{k=1}^{7}\alpha_k b_k(d_i) + \beta_1 w_i + \beta_2 T_i + \beta_3(w_i \cdot T_i) + \beta_4 H_i + \sum_{k=1}^{7}\gamma_k\bigl(w_i \cdot b_k(d_i)\bigr) \\
&\quad + \beta_5\,\text{turf}_i + \beta_6\,\text{snow\_sleet}_i + \beta_7\,\text{rain\_showers}_i + \beta_8\,\text{clock\_running}_i \\
&\quad + \beta_9\,\text{iced}_i + \beta_{10}\,\text{l2m}_i + \beta_{11}(\text{clock\_running}_i \cdot \text{l2m}_i) + \beta_{12}\,\text{playoffs}_i \\
&\quad + u_{s(i)} + v_{g(i)}
\end{aligned}$$

where $b_k(d_i)$ is the $k$-th B-spline basis function at distance $d_i$ (interior knots 28/38/48/58 yds, boundary knots 18/70 yds); $w_i$ = standardized wind speed; $T_i$ = standardized temperature; $H_i$ = standardized relative humidity; $u_{s(i)} \sim N(0, \sigma_s^2)$ is a kicker-season random intercept; $v_{g(i)} \sim N(0, \sigma_g^2)$ is a stadium random intercept.
...
|---|---:|---:|---:|---:|---:|---:|
| M0 (distance only) | 0.1045 | *TBD* | 0.774 | 0.1012 | *TBD* | 0.762 |
| M1 (full GLMM) | 0.1001 | *TBD* | 0.804 | **0.1007** | *TBD* | **0.769** |
| M2 (IPW) | **0.0984** | *TBD* | **0.805** | 0.1029 | *TBD* | 0.760 |
| M3 (augmented) | 0.1010 | *TBD* | 0.801 | 0.1017 | *TBD* | 0.767 |

: Model performance on in-sample (IS, $n = 11{,}548$) and held-out test set (OOS, $n = 2{,}251$). Brier and log-loss: lower is better. AUC: higher is better. Blocked kicks excluded. Bold indicates best value in column. Log-loss cells marked *TBD* are to be filled from the pending model re-run. {#tbl-model-metrics}
...
|---:|---:|---:|---:|---:|
| 0.05 | 0.1007 | *TBD* | 0.802 | 0.0139 |
| 0.10 | 0.1009 | *TBD* | 0.801 | 0.0170 |
| 0.25 | 0.1015 | *TBD* | 0.802 | 0.0286 |

: M3 weight sensitivity grid (PAT weight = Non-attempt weight). IS metrics only; OOS Brier for the primary 0.10 configuration is 0.1017. Log-loss cells marked *TBD* are to be filled from the pending model re-run. {#tbl-m3-grid}
