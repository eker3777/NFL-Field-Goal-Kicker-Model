...
Coaches decline potential kicking opportunities in favor of punting or going
for it on fourth down.

These decisions are not random. Actual attempts represent a selected group,
overrepresenting favorable kicking scenarios.
:::

::: {.column width="42%"}
::: {.takeaway}
If a coach believes they have information about their kicker's ability, that
belief can shape which kicks get attempted, information no outcome model
observes directly.
:::
:::
:::
...
- Frame the problem as what the model sees, not why coaches decide. If coaches
  act on a private read of their kicker, that read can shape which kicks are
  attempted and it never enters an outcome model as a variable, so the observed
  attempt set may be a non-representative slice of the kickable universe.
- This is a statement about what the data can and cannot show, not a measured
  claim about coach behavior. That matters, because by the end of the talk the
  data will point somewhere else.
...
- Held-out split is drawn at the game level, stratified by season: roughly 20%
  of each season's field-goal games, 593 disjoint games.
...
- Shaded band is the modeling window.
...
::: {.column width="25%"}
### [M3]{.model-m3} IPW
M1 fitted with stabilized IPW case weights.
...
- The screens are substantive, not cosmetic. Propensity at least 0.25 keeps out
  ...
  PAT range, so near-certain kicks are not labeled misses. We tested removing
...
- Natural spline on **field position** $\text{yl}_i$, full field 1 to 100
- **Log game seconds remaining** $\log(1 + t_i)$, quarter-time interactions
- Yards to go $\text{ytg}_i$; score state, venue, wind, temperature $\mathbf{S}_i$
- **Leverage** $L_i$, the win probability after a make minus the win probability
...
- Against 11,615 scored attempts the matching head agrees closely with realized
...
- $w_i$ Hajek-normalized to mean 1, capped at three seasonal standard
  deviations, renormalized
...
- Normalizing over the in-range subset rather than the full field matters for
  what M3 estimates. Normalizing over the full field would make M3 target a
...
:::: {.slidebody}
- Seven-column B-spline in distance ($d_i$). Interior knots 28, 38, 48, 58.
  Boundaries 18 and 70.
- Natural splines, 3 df each, in standardized wind ($W_i$), temperature ($T_i$),
  and humidity ($H_i$).
- Contextual indicators ($\mathbf{C}_i$). Turf, precipitation, clock running,
  last two minutes, iced, playoffs.
...
- Era stability and train/test balance are both in the appendix if they come up.
  Short version: M1 beats M3 in both halves of the window, and the realized
...
::: {.takeaway}
On long yards to go, field goals keep taking share from punts, in step with
kicker improvement. On short yards to go, going for it surges after about 2018,
a shift better kicking does not explain.
:::
...
::: {.takeaway}
Post-2020, coaches go for it more at nearly every field position. If declines
were about doubting the kicker, improving kickers should have moved these curves
the other way.
:::
...
::: {.takeaway}
On short yards to go, going for it carries positive win probability added almost
everywhere, while a field goal does not. The decline looks less like doubt about
the kicker and more like taking the higher-value option.
:::
...
- Coaches appear to have internalized this and acted on it.
...
3. IPW corrects what it claims to in-sample at long distance and still does not
   generalize. M1 is the better forecaster in both eras.
...
Kicker evaluation via an over-expected model. Coaching analysis via a
win-probability-maximizing decision model. A causal treatment of the attempt
decision.
:::
:::
::::
...
looks mostly like win maximization does not tell you much more about whether a
kick goes in.
...
## New Diamond Open Access journal {.qr}
...
![](figures/qr_code_jsdss.svg)

::: {.notes}
Quick plug for the journal. Diamond Open Access, no author or reader fees, edited
by Michael Schuckers. The code goes to the journal's site. 10 to 15 seconds.
:::
...
The realized split is an unremarkable draw.
:::
::::
...
1. Stabilize. $w_i = \bar{\pi} / \hat{\pi}(X_i)$, with $\hat{\pi}$ clipped to
   $[0.02, 0.98]$ before inversion.
2. Hajek-normalize to mean 1, then cap at
   $\bar{w}_s + 3\sigma_{w,s}$, then renormalize.
3. $\bar{\pi}$ and both normalizations are computed over the **in-range**
...
If asked why normalize in-range: normalizing over the full field would make M3
...
::: {.smaller}
Primary screen: propensity at least 0.25 and distance beyond 33 yards. 1,923 of
11,142 candidates admitted, with pseudo-miss labels biased in a known direction.
:::
:::
...
standardized wind, temperature and humidity, entered additively.
...
split by realized outcome. M3 pushes low-probability kicks leftward, which is
the same tail behavior the distance-band table shows, seen from a different
angle.
:::
...
Why did fourth-down behavior change so sharply around 2018 to 2020? Coaching
...
::: {.notes}
Explicitly labeled as speculation. If someone pushes on the coaching-tree
